...
! function() {
    class magnifying {
        ...
        init() {
            let _this = this;
            this.num = 6;
            $(".fangda").on("click", '#right', function() {
                let liwidth = $(this).siblings("#list").find("ul li").eq(0).outerWidth();
                let zonglen = $(this).siblings("#list").find("ul li").length;
                if (zonglen > _this.num) {
                    _this.num++;
                    _this.left.css('color', '#333');
                    if (_this.num === zonglen) {
                        _this.right.css('color', '#fff');
                    }
                    _this.ullist.animate({
                        left: -liwidth * (_this.num - 6)
                    });
                }
            })

            $(".fangda").on("click", '#left', function() {
                    let leftwidth = $(this).siblings("#list").find("ul li").eq(0).outerWidth();
                    let leftlen = $(this).siblings("#list").find("ul li").length;
                    ...
                    if (_this.num > 6) {
                        ...
                        _this.ullist.animate({
                            left: -leftwidth * (_this.num - 6)
                        });
                    }
                })
                // console.log(this.list);
            $("#list").on("click", 'li', function(ev) {
                let wu = $(this).find("img").attr("src");
                $(this).parents("#ulist").siblings("#spic").find(".xiao").attr("src", wu);
                $(this).parents("#ulist").siblings("#bf").find("#bpic").attr("src", wu);
            })
